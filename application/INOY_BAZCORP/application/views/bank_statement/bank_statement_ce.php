...
<script>
    $(document).ready(function(){
        $("#bs-date").jqxDateTimeInput({width: '250px', height: '25px'});

        <?php if(isset($is_edit)) :?>
        $("#bs-date").jqxDateTimeInput('val', <?php echo "'" . date( 'd/m/Y' , strtotime($data_edit[0]['date'])) . "'" ?>);
        <?php endif; ?>
    });
    ...
    function SaveData()
    {
        ...
        data_post['id_bank_statement'] = $("#id_bank_statement").val();
        data_post['date'] = $("#bs-date").val('date').format('yyyy-mm-dd');
        data_post['account'] = $("#coa-name").val().value;
        data_post['amount'] = $("#bs-amount").val();
        data_post['type'] = $("#bs-type").val();
        data_post['recipient'] = $("#bs-recipient").val();
        data_post['description'] = $("#bs-description").val();

        load_content_ajax(GetCurrentController(), 198, data_post);

    }

    function DiscardData()
    {
        load_content_ajax(GetCurrentController(), 194, null);
    }
...
<input type="hidden" id="id_bank_statement" value="<?php echo (isset($is_edit) ? $data_edit[0]['id_bank_statement'] : '') ?>" />
            ...
            <table class="table-form">
                ...
                <tr>
                    <td>
                        ...
                        <div class="column-input">
                            <input class="field" id="bs-amount" name="amount" type="text" value="<?php echo (isset($is_edit) ? $data_edit[0]['amount'] : '') ?>"
                        </div>
                    </td>
                    <td>
                        <div class="label">Type</div>
                        <div class="column-input">
                            <select id="bs-type" name="type">
                ...
                <tr>
                    <td>
                        ...
                        <div class="column-input">
                            <input class="field" id="bs-recipient" name="recipient" type="text" value="<?php echo (isset($is_edit) ? $data_edit[0]['recipient'] : '') ?>">
                        </div>
                    </td>
                    <td>
                        <div class="label">Description</div>
                        <div class="column-input">
                            <textarea id="bs-description" name="description"><?php echo (isset($is_edit) ? $data_edit[0]['description'] : '') ?></textarea>
                        </div>
                    </td>
                </tr>
                </tbody>
