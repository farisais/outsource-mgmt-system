...
$(document).ready(function(){
    ...
    $('#select-po-grid').on('rowdoubleclick', function (event) 
    {
        ...
        var urlHistory = "<?php echo base_url() ?>payment_receipt/get_payment_receipt_history?id_po=" + data.id_po ;
        var sourceHistory =
        {
            datatype: "json",
            datafields:
    ...
    $("#payment-history-grid").jqxGrid(
    {
        ...
        autoshowfiltericon: true,
        columns: [
            { text: 'Payment Number', dataField: 'payment_receipt_number'},
    ...
    function get_payment_left(id_po, id_payment)
    {
        ...
        var urlAjax = "<?php echo base_url() ?>payment_receipt/get_payment_left?id_po=" + id_po + payment;
        var data_post = {};
        $.ajax({
            url: urlAjax,
    		type: "POST",
    ...
    if(isset($is_edit))
    {?>                       
        get_payment_left(<?php echo $data_edit[0]['po'] ?>, <?php echo $data_edit[0]['id_payment_receipt'] ?>);
    <?php    
    } 
    ?> 
...
<div id='form-container' style="font-size: 13px; font-family: Arial, Helvetica, Tahoma">
    <div class="form-center" style="padding: 30px;">
        <div><h1 style="font-size: 18pt; font-weight: bold;">Payment Receipt / <span><?php echo (isset($is_edit) ? $data_edit[0]['payment_receipt_number'] : ''); ?></span></h1></div>
        <div>
